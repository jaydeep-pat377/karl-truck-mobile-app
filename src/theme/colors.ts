export const colors = {
  login: {
    light: {
      loadingBg: '#f0f4f8',
      inputBg: 'rgba(0,0,0,0.04)',
      inputBorder: 'rgba(0,0,0,0.08)',
      glassCardBg: '#FFFFFF',
      glassCardBorder: 'rgba(0,0,0,0.06)',
      decorativeCircle: 'rgba(69,139,0,0.15)',
      logoContainerBg: 'rgba(255,255,255,0.95)',
      brandTagline: '#808080',
      footerText: '#808080',
      footerBrand: '#458b00',
      circleOpacity1: 1,
      circleOpacity2: 1,
      circleOpacity3: 1,
      gradient: ['#FFFFFF', '#F8F9FA', '#F0F2F5', '#E8ECEF'],
    },
    dark: {
      loadingBg: '#0a0a0f',
      inputBg: 'rgba(255,255,255,0.05)',
      inputBorder: 'rgba(255,255,255,0.1)',
      glassCardBg: 'rgba(255,255,255,0.05)',
      glassCardBorder: 'rgba(255,255,255,0.18)',
      decorativeCircle: 'rgba(255,255,255,0.3)',
      logoContainerBg: 'rgba(255,255,255,0.95)',
      brandTagline: 'rgba(255,255,255,0.7)',
      footerText: 'rgba(255,255,255,0.6)',
      footerBrand: 'rgba(255,255,255,0.9)',
      circleOpacity1: 0.1,
      circleOpacity2: 0.08,
      circleOpacity3: 0.05,
      gradient: ['#0a0a0f', '#1a1a2e', '#16213e', '#0a0a0f'],
    },
    shadow: '#000000',
  },
  primary: {
    main: '#458b00',
    light: '#5aa510',
    dark: '#367000',
    contrast: '#FFFFFF',
  },
  secondary: {
    main: '#2f7ed8',
    light: '#5a9be8',
    dark: '#2565b0',
    contrast: '#FFFFFF',
  },
  grey: {
    100: '#000000',
    85: '#262626',
    80: '#323232',
    60: '#585957',
    50: '#808080',
    40: '#999999',
    25: '#BFBFBF',
    15: '#D9D9D9',
    10: '#E5E5E5',
    8: '#EBEBEB',
    5: '#F2F2F2',
    3: '#F7F7F7',
    light: '#F4F4F4',
  },
  progress: {
    trackLight: '#E8E8E8',
    trackDark: '#3A3A3A',
  },
  status: {
    prePour: '#f7bb00',
    inProcess: '#2f7ed8',
    completed: '#458b00',
    cancelled: '#c43926',
    delayed: '#c43926',
    onHold: '#585957',
    enRoute: '#8B5CF6',
    onSite: '#2f7ed8',
  },
  weather: {
    gradientStart: '#458b00',
    gradientEnd: '#5aa510',
    cloudyBg: '#585957',
  },
  success: {
    main: '#458b00',
    light: '#5aa510',
    dark: '#367000',
    background: '#e8f5e0',
  },
  warning: {
    main: '#f7bb00',
    light: '#ffd040',
    dark: '#d9a300',
    background: '#fef9e6',
  },
  error: {
    main: '#c43926',
    light: '#e05545',
    dark: '#a02d1d',
    background: '#fdecea',
  },
  info: {
    main: '#2f7ed8',
    light: '#5a9be8',
    dark: '#2565b0',
    background: '#e8f2fc',
  },
  dark: {
    background: '#000000',
    surface: '#262626',
    card: '#323232',
    cardElevated: '#3D3D3D',
    border: '#585957',
    text: {
      primary: '#FFFFFF',
      secondary: '#BFBFBF',
      disabled: '#808080',
      hint: '#999999',
    },
  },
  light: {
    background: '#F2F2F2',
    surface: '#FFFFFF',
    card: '#FFFFFF',
    cardElevated: '#F4F4F4',
    border: '#BFBFBF',
    text: {
      primary: '#000000',
      secondary: '#585957',
      disabled: '#BFBFBF',
      hint: '#808080',
    },
  },
  common: {
    white: '#FFFFFF',
    black: '#000000',
    transparent: 'transparent',
    shadow: '#000000',
  },
  overlay: {
    light: 'rgba(255, 255, 255, 0.1)',
    medium: 'rgba(0, 0, 0, 0.5)',
    dark: 'rgba(0, 0, 0, 0.7)',
    modal: 'rgba(0, 0, 0, 0.6)',
  },
  modal: {
    light: {
      cancelBg: '#F2F2F2',
      cancelText: '#585957',
      cancelBorder: '#E5E5E5',
    },
    dark: {
      cancelBg: '#404040',
      cancelText: '#FFFFFF',
      cancelBorder: '#5A5A5A',
    },
  },
  weatherTheme: {
    background: '#0A1628',
    cardBackground: '#111D2E',
    cardBorder: '#1E3A5F',
    gradient: {
      colors: ['#0A1628', '#0F1E32', '#162844', '#1E3A5F'] as const,
      locations: [0, 0.3, 0.6, 1] as const,
    },
    text: {
      primary: '#FFFFFF',
      secondary: '#94A3B8',
      hint: '#64748B',
    },
    menu: {
      background: '#111D2E',
      border: '#1E3A5F',
      overlay: 'rgba(0, 0, 0, 0.6)',
      shadow: '#000000',
      iconBackground: 'rgba(255, 255, 255, 0.06)',
      divider: 'rgba(255, 255, 255, 0.08)',
    },
  },
  weatherIcon: {
    sunGold: '#FFD700',
    sunOrange: '#FFA500',
    cloudLight: '#E0E7FF',
    cloudBlue: '#A5B4FC',
    rain: '#60A5FA',
    indicatorDot: '#FF6B6B',
  },
  productChip: {
    thermalCracking: '#FF4343',
    plasticCracking: '#EAAD29',
    cream: '#F5F5DC',
  },
  loader: {
    wheelSpoke: '#3A3B4A',
  },
  splash: {
    decorativeCircle1: 'rgba(255,255,255,0.08)',
    decorativeCircle2: 'rgba(255,255,255,0.05)',
    wheelSpokes: 'rgba(255,255,255,0.6)',
    subtitle: 'rgba(255,255,255,0.85)',
    loaderDots: 'rgba(255,255,255,0.9)',
    footer: 'rgba(255,255,255,0.6)',
  },
  brands: {
    googleMaps: '#4285F4',
    appleMaps: '#000000',
  },
  action: {
    call: {
      main: '#FF5722',
      light: '#FF8A65',
      dark: '#E64A19',
      bgLight: 'rgba(255, 87, 34, 0.1)',
      bgDark: 'rgba(255, 138, 101, 0.15)',
      borderLight: 'rgba(255, 87, 34, 0.2)',
    },
  },
  dashboard: {
    willCall: '#8B5CF6',
    inProgress: '#f7bb00',
    statGreen: '#458b00',
    statRed: '#c43926',
    statYellow: '#f7bb00',
    statBlue: '#2f7ed8',
  },
  regionCard: {
    light: {
      background: '#EBF5FB',
      progressColor: '#3498DB',
      progressBg: '#D4E6F1',
      badgeColor: '#F5A623',
      barColor: '#3498DB',
      titleColor: '#000000',
      qtyColor: '#000000',
      ofTextColor: '#000000',
      statsColor: '#000000',
    },
    dark: {
      background: '#1E3A5F',
      progressColor: '#3498DB',
      progressBg: '#85C1E9',
      badgeColor: '#F5A623',
      barColor: '#5DADE2',
      titleColor: '#FFFFFF',
      qtyColor: '#FFFFFF',
      ofTextColor: '#BFBFBF',
      statsColor: '#BFBFBF',
    },
  },
  mapMarker: {
    jobSite: '#c43926',
    truck: '#2f7ed8',
    plant: '#2f7ed8',
  },
  trackingStatus: {
    pending: '#6b7280',
    ticketed: '#6b7280',
    loading: '#FF9800',
    loaded: '#FF9800',
    toJob: '#8BC34A',
    atJob: '#4CAF50',
    pouring: '#009688',
    poured: '#009688',
    washing: '#03A9F4',
    toPlant: '#2196F3',
    atPlant: '#1565C0',
    cancelled: '#ef4444',
    voided: '#ef4444',
    live: '#4CAF50',
    remaining: '#E0E0E0',
    returning: '#2196F3',
  },
  chat: {
    light: {
      sentBubble: '#DCF8C6',
      receivedBubble: '#FFFFFF',
      textPrimary: '#111B21',
      timeText: 'rgba(0,0,0,0.45)',
      inputBg: 'rgba(0,0,0,0.06)',
      messageArea: '#F0F2F5',
    },
    dark: {
      sentBubble: '#005C4B',
      receivedBubble: '#1F2C34',
      textPrimary: '#E9EDEF',
      timeText: 'rgba(255,255,255,0.6)',
      inputBg: 'rgba(255,255,255,0.1)',
      background: '#0D1117',
      messageArea: '#0D1117',
    },
    whatsappGreen: '#00A884',
    readTick: '#53BDEB',
  },
  evaporation: {
    gradient: ['#458b00', '#5aa510', '#8db800', '#f7bb00', '#e8a000', '#d98500', '#c43926'],
    indicatorDot: '#FFFFFF',
  },
  infoIcons: {
    cyan: {
      light: '#00BCD4',
      dark: '#4DD0E1',
    },
    orange: {
      light: '#FF5722',
      dark: '#FF8A65',
    },
    purple: {
      light: '#673AB7',
      dark: '#B39DDB',
    },
    blue: {
      light: '#2f7ed8',
      dark: '#5a9be8',
    },
  },
  accent: {
    blue: '#3949ab',
    indigo: '#6366F1',
    indigoBg: 'rgba(99, 102, 241, 0.1)',
  },
  cardBg: {
    light: '#FAFAFA',
    dark: '#1a2a3a',
    border: '#E0E0E0',
    infoBg: '#E8F4FC',
  },
  tabBar: {
    light: {
      background: '#FFFFFF',
      iconInactive: '#8E8E93',
      iconActive: '#FFFFFF',
      labelActive: '#FFFFFF',
      border: '#E5E5EA',
    },
    dark: {
      background: '#4B5563',
      iconInactive: 'rgba(255,255,255,0.55)',
      iconActive: '#FFFFFF',
      labelActive: '#FFFFFF',
    },
  },
  statusBadge: {
    red: {
      text: '#c43926',
      textDark: '#e05545',
    },
    amber: {
      text: '#d9a300',
      textDark: '#f7bb00',
    },
    yellow: {
      text: '#92400E',
      textDark: '#FDE047',
    },
    green: {
      text: '#458b00',
      textDark: '#5aa510',
    },
    blue: {
      text: '#2f7ed8',
      textDark: '#5a9be8',
    },
    purple: {
      text: '#7C3AED',
      textDark: '#A78BFA',
    },
    gray: {
      text: '#4B5563',
      textDark: '#9CA3AF',
    },
  },
  weatherBadge: {
    sunny: '#f7bb00',
    partlyCloudy: '#6B7280',
    cloudy: '#9CA3AF',
    rain: '#2f7ed8',
    storm: '#6366F1',
    snow: '#5a9be8',
    fog: '#9CA3AF',
  },
  ticketBadge: {
    pending: {
      bg: 'rgba(255, 255, 255, 0.95)',
      text: '#666666',
      icon: '#888888',
    },
    ticketed: {
      bg: 'rgba(255, 255, 255, 0.95)',
      text: '#0288D1',
      icon: '#0288D1',
    },
    loading: {
      bg: 'rgba(255, 193, 7, 0.95)',
      text: '#5D4037',
      icon: '#5D4037',
    },
    loaded: {
      bg: 'rgba(255, 255, 255, 0.95)',
      text: '#0277BD',
      icon: '#0277BD',
    },
    toJob: {
      bg: 'rgba(33, 150, 243, 0.95)',
      text: '#FFFFFF',
      icon: '#FFFFFF',
    },
    atJob: {
      bg: 'rgba(255, 152, 0, 0.95)',
      text: '#FFFFFF',
      icon: '#FFFFFF',
    },
    pouring: {
      bg: 'rgba(255, 255, 255, 0.95)',
      text: '#458b00',
      icon: '#458b00',
    },
    washing: {
      bg: 'rgba(3, 169, 244, 0.95)',
      text: '#FFFFFF',
      icon: '#FFFFFF',
    },
    toPlant: {
      bg: 'rgba(156, 39, 176, 0.9)',
      text: '#FFFFFF',
      icon: '#FFFFFF',
    },
    atPlant: {
      bg: 'rgba(255, 255, 255, 0.95)',
      text: '#546E7A',
      icon: '#546E7A',
    },
    cancelled: {
      bg: 'rgba(196, 57, 38, 0.95)',
      text: '#FFFFFF',
      icon: '#FFFFFF',
    },
  },
  headerOverlay: {
    bg: 'rgba(255,255,255,0.15)',
    bgHover: 'rgba(255,255,255,0.2)',
    text: 'rgba(255,255,255,0.7)',
    textBright: 'rgba(255,255,255,0.8)',
    textBrightest: 'rgba(255,255,255,0.9)',
    border: 'rgba(255,255,255,0.2)',
  },
  scanner: {
    light: {
      overlay: 'rgba(0,0,0,0.6)',
      controlBackground: 'rgba(255,255,255,0.15)',
      textSecondary: 'rgba(255,255,255,0.7)',
      hintText: 'rgba(255,255,255,0.4)',
      feedbackOverlay: 'rgba(0,0,0,0.4)',
      flashlightActive: '#FBBF24',
      errorSheetHandle: '#E2E8F0',
      errorSheetTitle: '#000000',
      errorSheetMessage: '#585957',
    },
    dark: {
      overlay: 'rgba(0,0,0,0.6)',
      controlBackground: 'rgba(255,255,255,0.15)',
      textSecondary: 'rgba(255,255,255,0.7)',
      hintText: 'rgba(255,255,255,0.4)',
      feedbackOverlay: 'rgba(0,0,0,0.4)',
      flashlightActive: '#FBBF24',
      errorSheetHandle: '#585957',
      errorSheetTitle: '#FFFFFF',
      errorSheetMessage: '#BFBFBF',
    },
  },
  pdfViewer: {
    light: { webviewBg: '#F2F2F2' },
    dark: { webviewBg: '#1a1a2e' },
  },
  semiTransparent: {
    white03: 'rgba(255,255,255,0.03)',
    white05: 'rgba(255,255,255,0.05)',
    white06: 'rgba(255,255,255,0.06)',
    white08: 'rgba(255,255,255,0.08)',
    white10: 'rgba(255,255,255,0.1)',
    white12: 'rgba(255,255,255,0.12)',
    white15: 'rgba(255,255,255,0.15)',
    white20: 'rgba(255,255,255,0.2)',
    white30: 'rgba(255,255,255,0.3)',
    white40: 'rgba(255,255,255,0.4)',
    white60: 'rgba(255,255,255,0.6)',
    white70: 'rgba(255,255,255,0.7)',
    white75: 'rgba(255,255,255,0.75)',
    white50: 'rgba(255,255,255,0.5)',
    white55: 'rgba(255,255,255,0.55)',
    white80: 'rgba(255,255,255,0.8)',
    white85: 'rgba(255,255,255,0.85)',
    white90: 'rgba(255,255,255,0.9)',
    white92: 'rgba(255,255,255,0.92)',
    white95: 'rgba(255,255,255,0.95)',
    black02: 'rgba(0,0,0,0.02)',
    black04: 'rgba(0,0,0,0.04)',
    black05: 'rgba(0,0,0,0.05)',
    black06: 'rgba(0,0,0,0.06)',
    black10: 'rgba(0,0,0,0.1)',
    black15: 'rgba(0,0,0,0.15)',
    black20: 'rgba(0,0,0,0.2)',
    black30: 'rgba(0,0,0,0.3)',
    black40: 'rgba(0,0,0,0.4)',
    black45: 'rgba(0,0,0,0.45)',
    black50: 'rgba(0,0,0,0.5)',
    black60: 'rgba(0,0,0,0.6)',
    black80: 'rgba(0,0,0,0.8)',
    black90: 'rgba(0,0,0,0.9)',
    black95: 'rgba(0,0,0,0.95)',
    gray30: 'rgba(128,128,128,0.3)',
    darkGray90: 'rgba(30,30,30,0.9)',
    darkGray92: 'rgba(30,30,30,0.92)',
    darkGray95: 'rgba(30,30,30,0.95)',
    teal20: 'rgba(16,185,129,0.2)',
    green08: 'rgba(69,139,0,0.08)',
    green10: 'rgba(69,139,0,0.1)',
    green15: 'rgba(69,139,0,0.15)',
  },
  unloadingRate: {
    light: '#FF6B6B',
    medium: '#E53935',
    dark: '#B71C1C',
  },
  productStatus: {
    danger: '#c43926',
    safe: '#458b00',
    info: '#2f7ed8',
    truckIcon: '#5a9be8',
  },
  ticket: {
    status: {
      atPlant: {
        bg: '#ECEFF1',
        text: '#546E7A',
        iconBg: '#CFD8DC',
      },
      inTransit: {
        bg: '#e8f2fc',
        text: '#2f7ed8',
        iconBg: '#b8d4f0',
      },
      atSite: {
        bg: '#FFF3E0',
        text: '#E65100',
        iconBg: '#FFE0B2',
      },
      pouring: {
        bg: '#e8f5e0',
        text: '#458b00',
        iconBg: '#c8e6c0',
      },
      completed: {
        bg: '#e8f5e0',
        text: '#458b00',
        iconBg: '#a5d6a0',
      },
      returning: {
        bg: '#F3E5F5',
        text: '#7B1FA2',
        iconBg: '#E1BEE7',
      },
    },
    statusDark: {
      atPlant: {
        bg: 'rgba(150, 150, 150, 0.15)',
        text: '#AAAAAA',
        iconBg: 'rgba(150, 150, 150, 0.3)',
      },
      inTransit: {
        bg: 'rgba(47, 126, 216, 0.15)',
        text: '#5a9be8',
        iconBg: 'rgba(47, 126, 216, 0.3)',
      },
      atSite: {
        bg: 'rgba(255, 152, 0, 0.15)',
        text: '#FFB74D',
        iconBg: 'rgba(255, 152, 0, 0.3)',
      },
      pouring: {
        bg: 'rgba(69, 139, 0, 0.15)',
        text: '#5aa510',
        iconBg: 'rgba(69, 139, 0, 0.3)',
      },
      completed: {
        bg: 'rgba(69, 139, 0, 0.15)',
        text: '#5aa510',
        iconBg: 'rgba(69, 139, 0, 0.4)',
      },
      returning: {
        bg: 'rgba(156, 39, 176, 0.15)',
        text: '#BA68C8',
        iconBg: 'rgba(156, 39, 176, 0.3)',
      },
    },
    ui: {
      light: {
        timeText: '#666666',
        chevron: '#999999',
        filterBg: '#F5F5F5',
        filterBorder: '#E0E0E0',
        filterText: '#555555',
        progressBg: '#e8f5e0',
        emptyIcon: '#458b00',
      },
      dark: {
        timeText: 'rgba(255,255,255,0.5)',
        chevron: 'rgba(255,255,255,0.4)',
        filterBg: 'rgba(255,255,255,0.08)',
        filterBorder: 'rgba(255,255,255,0.12)',
        filterText: 'rgba(255,255,255,0.75)',
        progressBg: 'rgba(47, 126, 216, 0.15)',
        accentBlue: '#2f7ed8',
        accentBlueLight: '#5a9be8',
        badgeBg: 'rgba(47, 126, 216, 0.2)',
      },
    },
    emptyGradient: {
      light: ['#e8f5e0', '#c8e6c0'],
      dark: ['#2d5a00', '#458b00'],
    },
  },
  chart: {
    ordered: '#2f7ed8',
    poured: '#5aa510',
    delivered: {
      dark: '#FFFFFF',
      light: '#1a1a2e',
    },
    background: {
      dark: '#1a1a2e',
      light: '#FFFFFF',
    },
    tooltip: {
      text: '#FFFFFF',
      background: 'rgba(0,0,0,0.8)',
    },
    waiting: '#9CA3AF',
    pouring: '#458b00',
    washout: '#2f7ed8',
  },
  weatherParams: {
    temperature: '#4FC3F7',
    wind: '#81C784',
    pressure: '#FFB74D',
    dewPoint: '#4DD0E1',
    humidity: '#7986CB',
    evaporation: '#FF8A65',
  },
  text: {
    dark: '#333333',
    darkSecondary: '#666666',
    lightMuted: 'rgba(255,255,255,0.7)',
    lightSubtle: 'rgba(255,255,255,0.6)',
  },
  orderRequest: {
    status: {
      pending: 'rgb(47, 126, 216)',
      submitted: 'rgb(47, 126, 216)',
      approved: 'rgb(69, 139, 0)',
      rejected: 'rgb(196, 57, 38)',
      canceled: 'rgb(196, 57, 38)',
      fallback: 'rgb(75, 85, 99)',
    },
    orderType: {
      withProject: 'rgb(22, 163, 74)',
      withoutProjectWithProduct: 'rgb(147, 51, 234)',
      withoutProject: 'rgb(37, 99, 235)',
    },
    filterTab: {
      allColor: 'rgb(75, 85, 99)',
      activeBadgeBg: 'rgba(255,255,255,0.25)',
      light: {
        inactiveBg: 'rgba(0,0,0,0.04)',
        inactiveBadgeBg: 'rgba(0,0,0,0.06)',
      },
      dark: {
        inactiveBg: 'rgba(255,255,255,0.08)',
        inactiveBadgeBg: 'rgba(255,255,255,0.1)',
      },
    },
    card: {
      light: {
        border: 'rgba(0,0,0,0.06)',
        pillBg: 'rgba(0,0,0,0.04)',
      },
      dark: {
        border: 'rgba(255,255,255,0.06)',
        pillBg: 'rgba(255,255,255,0.06)',
      },
    },
  },
  orderRequestDetail: {
    chat: {
      ownBubbleBg: '#2563EB',
      ownTimeText: '#BFDBFE',
      light: {
        otherBubbleBg: '#F1F5F9',
        otherBorderColor: '#E2E8F0',
        ownSenderName: '#2563EB',
        otherSenderName: '#475569',
        roleLabel: '#94A3B8',
        separatorLine: '#E2E8F0',
        separatorText: '#94A3B8',
        otherMessageText: '#1E293B',
        otherTimeText: '#94A3B8',
      },
      dark: {
        ownSenderName: '#60A5FA',
      },
    },
    summaryAlert: {
      light: {
        bg: 'rgba(14, 165, 233, 0.08)',
        border: 'rgba(14, 165, 233, 0.25)',
        text: '#0369A1',
        textBold: '#0C4A6E',
      },
      dark: {
        bg: 'rgba(14, 165, 233, 0.12)',
        border: 'rgba(14, 165, 233, 0.3)',
        text: '#7DD3FC',
        textBold: '#7DD3FC',
      },
    },
    confirmModal: {
      light: {
        cardBg: '#FFFFFF',
        cancelBorder: '#E2E8F0',
      },
    },
    sectionHeader: {
      borderColor: 'rgba(128,128,128,0.2)',
    },
    input: {
      light: {
        bg: 'rgba(0,0,0,0.04)',
        border: 'rgba(0,0,0,0.1)',
      },
      dark: {
        bg: 'rgba(255,255,255,0.08)',
        border: 'rgba(255,255,255,0.15)',
      },
    },
    sendButton: {
      light: {
        disabledBg: 'rgba(0,0,0,0.06)',
      },
      dark: {
        disabledBg: 'rgba(255,255,255,0.1)',
      },
    },
    product: {
      light: {
        borderColor: 'rgba(0,0,0,0.06)',
      },
      dark: {
        borderColor: 'rgba(255,255,255,0.08)',
      },
    },
  },
  gradients: {
    light: {
      primary: ['#458b00', '#5aa510'],
      primarySubtle: ['#e8f5e0', '#f5fbf0'],
      secondary: ['#2f7ed8', '#5a9be8'],
      secondarySubtle: ['#e8f2fc', '#f0f8fe'],
      surface: ['#FFFFFF', '#F7F7F7'],
      header: ['#458b00', '#5aa510', '#FFFFFF'],
      topBackground: ['#367000', '#458b00', '#5aa510', '#e8f5e0', '#F2F2F2'],
      topBackgroundLocations: [0, 0.2, 0.4, 0.7, 1],
      orderDetails: ['#367000', '#458b00', '#5aa510', '#e8f5e0', '#F2F2F2'],
    },
    dark: {
      primary: ['#367000', '#458b00'],
      primarySubtle: ['#323232', '#262626'],
      secondary: ['#2565b0', '#2f7ed8'],
      secondarySubtle: ['#262626', '#1A1A1A'],
      surface: ['#323232', '#262626'],
      header: ['#367000', '#2d5a00', '#262626'],
      topBackground: ['#367000', '#458b00', '#323232', '#262626', '#000000'],
      topBackgroundLocations: [0, 0.25, 0.5, 0.75, 1],
      orderDetails: ['#367000', '#458b00', '#323232', '#262626', '#1E1E1E'],
    },
  },
  fallback: {
    segmentColor: '#6b7280',
  },
  detailHeader: {
    light: ['#FFFFFF', '#F8F8F8', '#F0F0F0'] as const,
    dark: ['#3A3A3A', '#2A2A2A', '#1E1E1E'] as const,
  },
  evaporationSeverity: {
    low: '#22C55E',
    moderate: '#F59E0B',
    high: '#F97316',
    critical: '#DC2626',
  },
  qrModal: {
    gradient: ['#7c3aed', '#6d28d9', '#4338ca'] as const,
    text: '#1e1b4b',
    loader: '#7c3aed',
    bodyBg: '#f8fafc',
    successDot: '#34d399',
    shadow: '#6d28d9',
    border: '#e2e8f0',
    activeBorder: '#7c3aed',
    pillBg: 'rgba(255,255,255,0.92)',
    liveBg: 'rgba(255,255,255,0.12)',
    liveBorder: 'rgba(255,255,255,0.18)',
  },
  eta: {
    main: '#F97316',
    bgLight: 'rgba(249,115,22,0.04)',
    bgDark: 'rgba(249,115,22,0.06)',
    textLight: '#7C2D12',
    textDark: '#FED7AA',
    border: 'rgba(249,115,22,0.15)',
    staleBg: 'rgba(217,119,6,0.15)',
    staleText: '#D97706',
  },
  weatherLink: {
    light: '#2563EB',
    dark: '#60A5FA',
  },
  chatToast: {
    light: {
      bg: '#FFFFFF',
      border: '#E2E8F0',
      sender: '#1E293B',
      message: '#64748B',
    },
    dark: {
      bg: '#1E293B',
      border: '#334155',
      sender: '#F1F5F9',
      message: '#94A3B8',
    },
  },
  calendarPicker: {
    light: {
      dayText: '#2d4150',
      disabled: '#d9e1e8',
      sectionTitle: '#b6c1cd',
    },
    dark: {
      dayText: '#FFFFFF',
      disabled: 'rgba(255,255,255,0.2)',
      sectionTitle: 'rgba(255,255,255,0.5)',
    },
    todayBgLight: 'rgba(76,175,80,0.1)',
    todayBgDark: 'rgba(76,175,80,0.15)',
  },
  svgFrame: {
    gray1: '#37393B',
    gray2: '#434345',
    gray3: '#4D4C4F',
    gray4: '#5E6062',
    gray5: '#7C7E80',
    gray6: '#8A8C8E',
    gray7: '#B0B2B4',
    teal: '#5E7789',
  },
  adIllustration: {
    truckGreen: '#4CAF50',
    truckGreenDark: '#2E7D32',
    boxOrange: '#FFB74D',
    boxOrangeStroke: '#E65100',
    wheel: '#424242',
    wheelInner: '#757575',
    road: '#E0E0E0',
    roadDash: '#BDBDBD',
    window: '#B3E5FC',
    windowStroke: '#0288D1',
    package: '#81D4FA',
    sunYellow: '#FFD54F',
    sunOrange: '#FF9800',
    sunRay: '#FFB300',
    grass: '#C8E6C9',
    cloud: '#E3F2FD',
  },
  adType: {
    delivery: {
      dark: ['#1B5E20', '#2E7D32', '#388E3C'] as const,
      light: ['#E8F5E9', '#C8E6C9', '#A5D6A7'] as const,
      accent: '#F57C00',
    },
    weather: {
      dark: ['#E65100', '#F57C00', '#FF9800'] as const,
      light: ['#FFF3E0', '#FFE0B2', '#FFCC80'] as const,
    },
    analytics: {
      dark: ['#1565C0', '#1976D2', '#2196F3'] as const,
      light: ['#E3F2FD', '#BBDEFB', '#90CAF9'] as const,
      accent: '#1976D2',
    },
  },
  trucksChart: {
    waiting: '#434348',
    pouring: '#90ed7d',
    grid: '#E0E0E0',
    gridDark: '#333333',
  },
  productionType: {
    company: '#8B5CF6',
    region: '#F97316',
    plant: '#3B82F6',
  },
  orderRequestSection: {
    verification: '#7C3AED',
    timeline: '#0EA5E9',
    details: '#14B8A6',
    products: '#F59E0B',
    summary: '#14B8A6',
    messages: '#8B5CF6',
    notes: '#F59E0B',
    scheduling: '#6366F1',
  },
  scanDetails: {
    light: {
      tint: 'rgba(69,139,0,0.08)',
      tintStrong: 'rgba(69,139,0,0.1)',
    },
    dark: {
      tint: 'rgba(69,139,0,0.15)',
      tintStrong: 'rgba(69,139,0,0.2)',
      dangerBg: 'rgba(196,57,38,0.15)',
    },
  },
  workspaceSwitcher: {
    errorBg: '#FEF2F2',
  },
  evaporationPill: {
    unknownBg: '#9CA3AF',
  },
  qrStatus: {
    cancelled: '#ef4444',
    atPlant: '#10b981',
    pouring: '#f59e0b',
    washing: '#0ea5e9',
    toPlant: '#3b82f6',
    atJob: '#6366f1',
    loading: '#8b5cf6',
    fallback: '#94a3b8',
  },
};

export const statusColorMap: Record<string, string> = {
  PRE_POUR: colors.success.main,
  'Pre-Pour': colors.success.main,
  'pre_pour': colors.success.main,
  'Pre Pour': colors.success.main,
  'pre pour': colors.success.main,
  NORMAL: colors.success.main,
  'Normal': colors.success.main,
  'normal': colors.success.main,
  IN_PROCESS: colors.success.main,
  'In Progress': colors.success.main,
  'in progress': colors.success.main,
  'in_process': colors.success.main,
  COMPLETED: colors.success.main,
  'Completed': colors.success.main,
  'completed': colors.success.main,
  CANCELLED: colors.error.main,
  CANCELED: colors.error.main,
  'Cancelled': colors.error.main,
  'Canceled': colors.error.main,
  'cancelled': colors.error.main,
  'canceled': colors.error.main,
  DELAYED: colors.error.main,
  'Delayed': colors.error.main,
  'delayed': colors.error.main,
  HOLD: colors.error.main,
  'Hold': colors.error.main,
  'hold': colors.error.main,
  ON_HOLD: colors.error.main,
  'On Hold': colors.error.main,
  'on hold': colors.error.main,
  'Hold Delivery': colors.error.main,
  'hold delivery': colors.error.main,
  'hold_delivery': colors.error.main,
  ENRT: colors.status.enRoute,
  ONSIT: colors.status.onSite,
  WILL_CALL: colors.warning.main,
  'Will Call': colors.warning.main,
  'will call': colors.warning.main,
  'will_call': colors.warning.main,
  WEATHER_PERMITTING: colors.warning.main,
  'Weather Permitting': colors.warning.main,
  'weather permitting': colors.warning.main,
  'weather_permitting': colors.warning.main,
  WAIT_LIST: colors.warning.main,
  'Wait List': colors.warning.main,
  'wait list': colors.warning.main,
  'wait_list': colors.warning.main,
  'Waitlist': colors.warning.main,
  'waitlist': colors.warning.main,
};

export const getPerformanceColor = (performancePercent: number): string => {
  if (performancePercent >= 90) {
    return colors.success.main;
  } else if (performancePercent >= 60) {
    return colors.warning.main;
  } else {
    return colors.error.main;
  }
};

export type ColorTheme = 'light' | 'dark';
