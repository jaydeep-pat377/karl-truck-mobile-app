...
export const OrderRequestListScreen: React.FC = () => {
  ...
  const FILTER_TABS: FilterTab[] = useMemo(() => [
    { key: 'all', label: t('orderRequests.list.filters.all'), color: colors.orderRequest.filterTab.allColor },
    { key: 'pending', label: t('orderRequests.list.filters.pending'), color: STATUS_COLORS.pending },
    { key: 'submitted', label: t('orderRequests.list.filters.submitted'), color: STATUS_COLORS.submitted },
    { key: 'approved', label: t('orderRequests.list.filters.accepted'), color: STATUS_COLORS.approved },
    { key: 'rejected', label: t('orderRequests.list.filters.rejected'), color: STATUS_COLORS.rejected },
  ], [t]);
    ...
    <View style={styles.searchContainer}>
      ...
      >
        <TextInput
          ...
          placeholder={t('orderRequests.list.searchPlaceholder')}
  ...
  const renderOrderCard = useCallback(
    ({ item }: { item: OrderEntity }) => {
      ...
      const orderTypeLabelMap: Record<string, string> = {
        with_project: t('orderRequests.list.orderType.project'),
        without_project_with_product: t('orderRequests.list.orderType.woProjectWithProduct'),
        without_project: t('orderRequests.list.orderType.woProject'),
      };
      const orderTypeLabel =
        orderTypeLabelMap[item.order_type] ?? t('orderRequests.list.orderType.unknown');
      const orderTypeIcon =
      ...
      const statusLabel =
        statusLabelMap[item.status] ?? item.status.charAt(0).toUpperCase() + item.status.slice(1);
      const orderStatusLabel =
        item.order_status != null
          ? ORDER_STATUS_LABELS[item.order_status] ?? ''
          : '';
      const displayName =
        item.job_name || item.job_address || t('orderRequests.list.untitledOrder');
              ...
              {item.quantity != null && (
                <View style={styles.metaItem}>
                  ...
                  <Text
                    ...
                    numberOfLines={1}
                  >
                    {t('orderRequests.list.quantityCy', { value: Number(item.quantity).toFixed(2) })}
                  </Text>
                </View>
              )}
            </View>
  ...
  const renderEmpty = () => {
    ...
    return (
      <EmptyView
        ...
        title={t('orderRequests.list.empty.title')}
        subtitle={
          searchText
            ? t('orderRequests.list.empty.searchSubtitle')
            : t('orderRequests.list.empty.subtitle')
        }
      />
    );
  };
  ...
  return (
    <ScreenContainer edges={[]} usePlainView={false}>
      {/* Header */}
      <ScreenHeader
        title={t('orderRequests.list.title')}
        showBackButton
        showRefreshButton
          ...
          <View style={styles.headerInfoSection}>
              ...
              >
                ...
                <View style={styles.createButtonContent}>
                  <Text variant="body" style={styles.createButtonTitle}>
                    {t('orderRequests.list.createNew')}
                  </Text>
                  <Text variant="captionSmall" style={styles.createButtonSubtitle}>
                    {t('orderRequests.list.tapToSubmit')}
                  </Text>
                </View>
              </TouchableOpacity>
            ...
            <View style={styles.subtitleRow}>
              ...
              >
                ...
                >
                  {t('orderRequests.list.totalCount', { count: counts.total })}
                </Text>
              </View>
            </View>
          </View>
