              ...
              <View style={styles.emptyList}>
                <Text variant="body" color="hint">
                  {isServerSearch && search.length < 2
                    ? t('orderRequests.create.typeAtLeastTwo')
                    : t('orderRequests.create.noResults')}
                </Text>
              </View>
            }
            ...
            <View style={{ padding: ms(12), borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: themeColors.border }}>
              ...
              >
                <Text variant="buttonSmall" style={{ color: colors.common.white, fontWeight: '700' }}>
                  {t('common.done')}{localSelected.length > 0 ? ` (${localSelected.length})` : ''}
                </Text>
              </TouchableOpacity>
            </View>
          )}
      ...
      <View style={themedAlertStyles.overlay}>
        <View style={[themedAlertStyles.card, { backgroundColor: cardBg }]}>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
          >
            <Text variant="buttonSmall" style={{ color: colors.common.white, fontWeight: '700' }}>{t('orderRequests.detail.ok')}</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
...
export const CreateOrderRequestScreen: React.FC = () => {
  ...
  const spacingTypeLabel = useMemo(() => {
    if (spacingType === 'yards_per_hour') return t('orderRequests.create.spacingTypes.yardsPerHour');
    if (spacingType === 'minutes') return t('orderRequests.create.spacingTypes.minutes');
    return '';
  }, [spacingType, t]);
  ...
  const validate = useCallback((): boolean => {
    ...
    // Base required
    if (!companyId) missing.push(t('orderRequests.create.fields.company'));
    if (showRegion && !regionCode) missing.push(t('orderRequests.create.fields.region'));
    if (!usageCode) missing.push(t('orderRequests.create.fields.usage'));
    if (orderStatus === null || orderStatus === undefined) missing.push(t('orderRequests.create.fields.orderStatus'));
    if (!onJobDate) missing.push(orderType === 'with_project' ? t('orderRequests.create.fields.onJobDate') : t('orderRequests.create.fields.requestedOnJobDate'));
    if (!onJobTime) missing.push(orderType === 'with_project' ? t('orderRequests.create.fields.onJobTime') : t('orderRequests.create.fields.requestedOnJobTime'));
    if (!jobAddress) missing.push(t('orderRequests.create.fields.jobAddress'));
    if (!jobCity) missing.push(t('orderRequests.create.fields.jobCity'));
    if (!contactName) missing.push(t('orderRequests.create.fields.contactName'));
    if (!contactPhone) missing.push(t('orderRequests.create.fields.contactPhone'));
    if (!slump) missing.push(t('orderRequests.create.fields.slump'));
    if (!quantity) missing.push(t('orderRequests.create.fields.quantity'));

    // Type-specific required
    if (orderType === 'with_project') {
      if (!projectCode) missing.push(t('orderRequests.create.fields.project'));
    } else if (orderType === 'without_project_with_product') {
      if (!concreteProductCode) missing.push(t('orderRequests.create.fields.product'));
    } else if (orderType === 'without_project') {
      if (!knowMixCode) {
        if (!psi) missing.push(t('orderRequests.create.fields.psi'));
        if (!rockSize) missing.push(t('orderRequests.create.fields.rockSize'));
        if (!airNonAir) missing.push(t('orderRequests.create.fields.airNonAir'));
        if (!flyAsh) missing.push(t('orderRequests.create.fields.flyAsh'));
      }
    }
    ...
    if (missing.length > 0) {
      setThemedAlert({
        visible: true,
        type: 'warning',
        title: t('orderRequests.create.missingRequired'),
        message: t('orderRequests.create.missingRequiredMessage', { fields: missing.join('\n') }),
      });
      return false;
    }
    return true;
  }, [companyId, regionCode, usageCode, onJobDate, onJobTime, jobAddress, jobCity, contactName, contactPhone,
    slump, quantity, orderType, projectCode, concreteProductCode, knowMixCode, psi, rockSize, airNonAir, flyAsh, t]);
    ...
    try {
      if (isEditMode && editOrderId) {
        ...
        setThemedAlert({
          visible: true,
          type: 'success',
          title: t('orderRequests.create.successTitle'),
          message: t('orderRequests.create.updateSuccess'),
          onDismiss: () => {
        ...
        setThemedAlert({
          ...
          type: 'success',
          title: t('orderRequests.create.successTitle'),
          message: t('orderRequests.create.createSuccess'),
  ...
  const localizedOrderTypeOptions = useMemo(() => [
    { key: 'with_project' as OrderType, label: t('orderRequests.create.orderTypes.withProject'), icon: 'clipboard-check' },
    { key: 'without_project' as OrderType, label: t('orderRequests.create.orderTypes.withoutProject'), icon: 'package-variant' },
    { key: 'without_project_with_product' as OrderType, label: t('orderRequests.create.orderTypes.withoutProjectWithProduct'), icon: 'clipboard-list' },
  ], [t]);

  const localizedOrderStatusOptions: DropdownOption[] = useMemo(() => [
    { value: '0', label: t('orderRequests.create.orderStatus.normal') },
    { value: '1', label: t('orderRequests.create.orderStatus.willCall') },
    { value: '2', label: t('orderRequests.create.orderStatus.weatherPermitting') },
    { value: '3', label: t('orderRequests.create.orderStatus.hold') },
    { value: '4', label: t('orderRequests.create.orderStatus.completed') },
    { value: '5', label: t('orderRequests.create.orderStatus.waitList') },
  ], [t]);

  const localizedSpacingTypeOptions: DropdownOption[] = useMemo(() => [
    { value: 'yards_per_hour', label: t('orderRequests.create.spacingTypes.yardsPerHour') },
    { value: 'minutes', label: t('orderRequests.create.spacingTypes.minutes') },
  ], [t]);
  ...
  const getDropdownOptions = useCallback((): DropdownOption[] => {
    switch (activeDropdown) {
      ...
      case 'orderStatus':
        return localizedOrderStatusOptions;
      case 'spacingType':
        return localizedSpacingTypeOptions;
      case 'airNonAir':
        return AIR_OPTIONS;
      case 'psi':
        return PSI_OPTIONS;
  ...
  }, [activeDropdown, customerOptions, regionOptions, projectOptions, productOptions, referencedOrderOptions, admixtureOptions, otherOptions, localizedOrderStatusOptions, localizedSpacingTypeOptions]);

  const getDropdownTitle = useCallback((): string => {
    switch (activeDropdown) {
      case 'company': return t('orderRequests.create.dropdowns.selectCompany');
      case 'region': return t('orderRequests.create.dropdowns.selectRegion');
      case 'project': return t('orderRequests.create.dropdowns.selectProject');
      case 'product': return t('orderRequests.create.dropdowns.selectConcreteProduct');
      case 'referencedOrder': return t('orderRequests.create.dropdowns.selectReferencedOrder');
      case 'usage': return t('orderRequests.create.dropdowns.selectUsage');
      case 'orderStatus': return t('orderRequests.create.dropdowns.selectOrderStatus');
      case 'spacingType': return t('orderRequests.create.dropdowns.selectSpacingType');
      case 'airNonAir': return t('orderRequests.create.dropdowns.selectAirNonAir');
      case 'psi': return t('orderRequests.create.dropdowns.selectPsi');
      case 'rockSize': return t('orderRequests.create.dropdowns.selectRockSize');
      case 'flyAsh': return t('orderRequests.create.dropdowns.selectFlyAsh');
      case 'slump': return t('orderRequests.create.dropdowns.selectSlump');
      case 'callBackLoad': return t('orderRequests.create.dropdowns.selectCallBackLoad');
      case 'admixtureProduct': return t('orderRequests.create.dropdowns.selectAdmixtureProduct');
      case 'otherProduct': return t('orderRequests.create.dropdowns.selectOtherProduct');
      default: return t('orderRequests.detail.select');
    }
  }, [activeDropdown, t]);
    ...
    <View style={styles.fieldContainer}>
      ...
      >
        <Text
          ...
          numberOfLines={1}
        >
          {displayValue || t('orderRequests.create.selectField', { field: label })}
        </Text>
        <Icon
          name="chevron-down"
          size={ms(20)}
  ...
  if (isFormDataLoading || (isEditMode && isEditLoading)) {
    return (
      <ScreenContainer edges={[]}>
        <ScreenHeader
          title={isEditMode ? t('orderRequests.create.editTitle') : t('orderRequests.create.title')}
        />
        <View style={styles.loadingContainer}>
          <TruckLoader size={120} message={t('orderRequests.create.loadingFormData')} color="dark" />
        </View>
      </ScreenContainer>
    );
  }
    ...
    <ScreenContainer edges={[]}>
      ...
      */}
      ...
      >
          ...
          {isReadOnly && (
            <View style={[styles.banner, { backgroundColor: colors.warning.background }]}>
              ...
              <Text variant="bodySmall" style={{ color: colors.warning.dark, marginLeft: spacing.sm, flex: 1 }}>
                {t('orderRequests.create.readOnlyBanner', { status: editOrder?.status })}
              </Text>
            </View>
          )}
          ...
          <View style={[styles.section, { backgroundColor: themeColors.surface, borderColor: themeColors.border }]}>
            <Text variant="bodyLarge" style={[styles.sectionTitle, { fontWeight: '700' }]}>
              {t('orderRequests.create.sections.orderType')}
            </Text>
            <View style={styles.orderTypeToggleRow}>
              {localizedOrderTypeOptions.map((opt) => {
          ...
          <View style={[styles.section, { backgroundColor: themeColors.surface, borderColor: themeColors.border }]}>
            <View style={styles.sectionHeaderRow}>
              ...
              <Text variant="bodyLarge" style={styles.sectionHeaderTitle}>
                {t('orderRequests.create.sections.jobInformation')}
              </Text>
            </View>

            {renderDropdownField(t('orderRequests.create.fields.company'), companyName ? `${companyName} (${companyId})` : '', 'company', true)}
            {orderType === 'with_project' && (
              <>
                {renderDropdownField(t('orderRequests.create.fields.project'), projectName ? `${projectName} (${projectCode})` : '', 'project', true)}
              </>
            )}
            {renderDropdownField(t('orderRequests.create.fields.referencedOrder'), referencedOrderLabel || referencedOrder || '', 'referencedOrder')}
            {showRegion && renderDropdownField(t('orderRequests.create.fields.region'), regionName ? `${regionName} (${regionCode})` : '', 'region', true)}
            {renderField(t('orderRequests.create.fields.customerJobNumber'), customerJobNumber, setCustomerJobNumber, t('orderRequests.create.placeholders.customerJobNumber'))}
            {renderDropdownField(t('orderRequests.create.fields.usage'), usageCode || '', 'usage', true)}
            {renderField(t('orderRequests.create.fields.poNumber'), poNumber, setPoNumber, t('orderRequests.create.placeholders.poNumber'))}
            {renderDropdownField(t('orderRequests.create.fields.orderStatus'), orderStatusLabel, 'orderStatus', true)}

            {/* On Job Date - with calendar picker */}
            <View style={styles.fieldContainer}>
              {renderLabel(
                orderType === 'with_project' ? t('orderRequests.create.fields.onJobDate') : t('orderRequests.create.fields.requestedOnJobDate'),
                true,
              )}
              <TouchableOpacity
                style={[
              ...
              >
                <Text
                  ...
                  style={{ flex: 1 }}
                >
                  {onJobDate || t('orderRequests.detail.selectDate')}
                </Text>
              ...
              <CalendarPickerModal
                ...
                isDark={isDark}
                title={orderType === 'with_project' ? t('orderRequests.create.fields.onJobDate') : t('orderRequests.create.fields.requestedOnJobDate')}
              />
            </View>
            ...
            <View style={styles.fieldContainer}>
              {renderLabel(
                orderType === 'with_project' ? t('orderRequests.create.fields.onJobTime') : t('orderRequests.create.fields.requestedOnJobTime'),
                true,
              )}
              <TouchableOpacity
                style={[
              ...
              >
                <Text
                  ...
                  style={{ flex: 1 }}
                >
                  {onJobTime ? (() => { const [h, m] = onJobTime.split(':').map(Number); const period = h >= 12 ? 'PM' : 'AM'; const h12 = h === 0 ? 12 : h > 12 ? h - 12 : h; return `${h12}:${String(m).padStart(2, '0')} ${period}`; })() : t('orderRequests.detail.selectTime')}
                </Text>
            ...
            {renderField(t('orderRequests.create.fields.jobName'), jobName, setJobName, t('orderRequests.create.placeholders.jobName'))}
          </View>
          ...
          <View style={[styles.section, { backgroundColor: themeColors.surface, borderColor: themeColors.border }]}>
            <View style={styles.sectionHeaderRow}>
              ...
              <Text variant="bodyLarge" style={styles.sectionHeaderTitle}>
                {t('orderRequests.create.sections.jobLocation')}
              </Text>
            </View>
            {renderField(t('orderRequests.create.fields.jobAddress'), jobAddress, setJobAddress, t('orderRequests.create.placeholders.jobAddress'), true)}
            {renderField(t('orderRequests.create.fields.jobCity'), jobCity, setJobCity, t('orderRequests.create.placeholders.jobCity'), true)}
            {renderField(t('orderRequests.create.fields.jobState'), jobState, setJobState, t('orderRequests.create.placeholders.jobState'))}
            {renderField(t('orderRequests.create.fields.jobZipCode'), jobZipCode, setJobZipCode, t('orderRequests.create.placeholders.jobZipCode'), false, {
          ...
          <View style={[styles.section, { backgroundColor: themeColors.surface, borderColor: themeColors.border }]}>
            <View style={styles.sectionHeaderRow}>
              ...
              <Text variant="bodyLarge" style={styles.sectionHeaderTitle}>
                {t('orderRequests.create.sections.jobsiteContact')}
              </Text>
            </View>
            {renderField(t('orderRequests.create.fields.contactName'), contactName, setContactName, t('orderRequests.create.placeholders.contactName'), true)}
            {renderField(t('orderRequests.create.fields.contactPhone'), contactPhone, setContactPhone, t('orderRequests.create.placeholders.contactPhone'), true, {
          ...
          <View style={[styles.section, { backgroundColor: themeColors.surface, borderColor: themeColors.border }]}>
            <View style={styles.sectionHeaderRow}>
              ...
              <Text variant="bodyLarge" style={styles.sectionHeaderTitle}>
                {t('orderRequests.create.sections.driverInstructions')}
              </Text>
            </View>
            <View style={styles.fieldContainer}>
              {renderTextInput(
                driverInstructions,
                setDriverInstructions,
                t('orderRequests.create.placeholders.driverInstructions'),
                { multiline: true },
              )}
            </View>
          </View>
          ...
          <View style={[styles.section, { backgroundColor: themeColors.surface, borderColor: themeColors.border }]}>
            <View style={styles.sectionHeaderRow}>
              ...
              <Text variant="bodyLarge" style={styles.sectionHeaderTitle}>
                {t('orderRequests.create.sections.concreteProduct')}
              </Text>
            </View>
            ...
            {orderType === 'with_project' && (
              <>
                {renderDropdownField(t('orderRequests.create.fields.product'), concreteProductName || '', 'product', true)}
                {renderField(
                  t('orderRequests.create.fields.nonProjectProduct'),
                  concreteProductText,
                  setConcreteProductText,
                  t('orderRequests.create.placeholders.specificMixCode'),
                )}
              </>
            )}
            ...
            {orderType === 'without_project' && (
              <>
                ...
                <View style={styles.fieldContainer}>
                  {renderLabel(t('orderRequests.create.knowMixCode'))}
                ...
                {renderField(
                  t('orderRequests.create.fields.concreteProduct'),
                  concreteProductText,
                  setConcreteProductText,
                  t('orderRequests.create.placeholders.specificMixCode'),
                )}
                ...
                {!knowMixCode && (
                  <>
                    {renderDropdownField(t('orderRequests.create.fields.psi'), psi, 'psi', true)}
                    {renderDropdownField(t('orderRequests.create.fields.rockSize'), rockSize, 'rockSize', true)}
                    {renderDropdownField(t('orderRequests.create.fields.airNonAir'), airNonAir, 'airNonAir', true)}
                    {renderDropdownField(t('orderRequests.create.fields.flyAsh'), flyAsh, 'flyAsh', true)}
                  </>
                )}
              </>
            )}
            ...
            {renderDropdownField(t('orderRequests.create.fields.slump'), slump, 'slump', true)}

            {renderField(t('orderRequests.create.fields.quantity'), quantity, setQuantity, t('orderRequests.create.placeholders.quantity'), true, {
              keyboardType: 'decimal-pad',
            })}

            <View style={styles.rowFields}>
              <View style={{ flex: 1, marginRight: spacing.sm }}>
                {renderDropdownField(t('orderRequests.create.fields.spacingType'), spacingTypeLabel, 'spacingType', true)}
              </View>
              <View style={{ flex: 1, marginLeft: spacing.sm }}>
                {renderField(
                  spacingType === 'yards_per_hour' ? t('orderRequests.create.fields.spacingYardsPerHour') : t('orderRequests.create.fields.spacingMinutes'),
                  truckSpacing,
                  setTruckSpacing,
                  spacingType === 'yards_per_hour' ? t('orderRequests.create.placeholders.yardsPerHour') : t('orderRequests.create.placeholders.minutes'),
                  true,
                  { keyboardType: 'decimal-pad' },
                )}
              </View>
            ...
            {renderField(t('orderRequests.create.fields.concreteNotes'), concreteNotes, setConcreteNotes, t('orderRequests.create.placeholders.concreteNotes'), false, {
              multiline: true,
            })}
            {renderDropdownField(t('orderRequests.create.fields.callBackLoad'), callBackLoad || '', 'callBackLoad')}
          </View>
          ...
          <View style={[styles.section, { backgroundColor: themeColors.surface, borderColor: themeColors.border }]}>
            <View style={styles.sectionHeaderRow}>
              ...
              <Text variant="bodyLarge" style={styles.sectionHeaderTitle}>
                {t('orderRequests.create.sections.admixtureProduct')}
              </Text>
            </View>
            {renderDropdownField(t('orderRequests.create.fields.product'), admixtureProductName || '', 'admixtureProduct')}
            {renderField(t('orderRequests.create.fields.admixtureNotes'), admixtureNotes, setAdmixtureNotes, t('orderRequests.create.placeholders.admixtureNotes'), false, {
              multiline: true,
            })}
          </View>
          ...
          <View style={[styles.section, { backgroundColor: themeColors.surface, borderColor: themeColors.border }]}>
            <View style={styles.sectionHeaderRow}>
              ...
              <Text variant="bodyLarge" style={styles.sectionHeaderTitle}>
                {t('orderRequests.create.sections.otherProduct')}
              </Text>
            </View>
            {renderDropdownField(t('orderRequests.create.fields.product'), otherProductName || '', 'otherProduct')}
            {renderField(t('orderRequests.create.fields.otherNotes'), otherNotes, setOtherNotes, t('orderRequests.create.placeholders.otherNotes'), false, {
              multiline: true,
            })}
          </View>
                ...
                >
                  {isMutating ? (
                    ...
                    <Text variant="body" style={{ color: isFormValid ? colors.common.white : (isDark ? colors.dark.text.hint : colors.grey[50]), fontWeight: '700', fontSize: ms(16) }}>
                      {isEditMode ? t('orderRequests.create.update') : t('orderRequests.create.send')}
                    </Text>
                  )}
                </TouchableOpacity>
              </View>
      ...
      <SearchableDropdownModal
        ...
        searchPlaceholder={
          activeDropdown === 'referencedOrder'
            ? t('orderRequests.create.searchPlaceholders.referencedOrder')
            : activeDropdown === 'product'
            ? t('orderRequests.create.searchPlaceholders.product')
            : activeDropdown === 'admixtureProduct'
            ? t('orderRequests.create.searchPlaceholders.admixture')
            : activeDropdown === 'otherProduct'
            ? t('orderRequests.create.searchPlaceholders.otherProduct')
            : undefined
        }
