...
export const TicketScanHistoryScreen: React.FC = () => {
  ...
  const handleDeleteItem = useCallback(
    (item: ScanRecord) => {
      ...
      Alert.alert(t('ticketScan.history.deleteScanTitle'), t('ticketScan.history.deleteScanMessage'), [
  ...
  const handleClearHistory = useCallback(() => {
    Alert.alert(t('ticketScan.history.clearHistoryTitle'), t('ticketScan.history.clearHistoryMessage'), [
      { text: t('common.cancel'), style: 'cancel' },
      {
        text: t('ticketScan.history.clearAll'),
        style: 'destructive',
        onPress: async () => {
  ...
  const formatDate = (timestamp: number) => {
    ...
    if (diffMins < 1) return t('ticketScan.history.justNow');
    if (diffMins < 60) return t('ticketScan.history.minutesAgo', { count: diffMins });
    if (diffHours < 24) return t('ticketScan.history.hoursAgo', { count: diffHours });
    if (diffDays < 7) return t('ticketScan.history.daysAgo', { count: diffDays });
  ...
  const renderItem = ({ item }: { item: ScanRecord }) => {
    ...
    const title = isTicket
      ? t('ticketScan.history.ticketWithCode', { code: ticketCode || '\u2014' })
      : isTruck
        ? t('ticketScan.history.truckWithCode', { code: truckCode || '\u2014' })
        : item.data;
    ...
    return (
      <View style={styles.emptyContainer}>
        ...
        <Text variant="body" style={{ fontWeight: '600', marginBottom: ms(4) }}>
          {t('ticketScan.history.noScansTitle')}
        </Text>
        <Text variant="caption" color="hint" style={{ textAlign: 'center' }}>
          {t('ticketScan.history.noScansSubtitle')}
        </Text>
      </View>
    );
  };
    ...
    return (
      <View style={styles.footerLoader}>
        ...
        <Text variant="caption" color="secondary" style={{ marginLeft: spacing.sm }}>
          {t('ticketScan.history.loadingMore')}
        </Text>
      </View>
    );
  };
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: themeColors.background }]} edges={['top', 'bottom']}>
        <View style={styles.header}>
          ...
          <Text variant="h2">{t('ticketScan.history.title')}</Text>
      ...
      {totalCount > 0 && (
        <View style={styles.subHeader}>
          <Text variant="caption" color="secondary">
            {t('ticketScan.history.scanCount', { count: totalCount })}
          </Text>
