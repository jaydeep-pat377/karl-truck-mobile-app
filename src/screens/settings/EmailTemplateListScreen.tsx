...
export const EmailTemplateListScreen: React.FC = () => {
  ...
  const sections: Section[] = useMemo(() => {
    ...
    defaults.forEach((def) => {
      const category = def.category || t('emailTemplate.categories.general');
  ...
  const getStatusBadge = (item: TemplateRow) => {
    if (item.customTemplate) {
      if (item.customTemplate.is_active) {
        return { label: t('emailTemplate.status.customized'), bgColor: colors.primary.main + '18', textColor: colors.primary.main };
      }
      return { label: t('emailTemplate.status.inactive'), bgColor: themeColors.border + '60', textColor: themeColors.text.hint };
    }
    return { label: t('emailTemplate.status.default'), bgColor: isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)', textColor: themeColors.text.secondary };
  };
        ...
        {isCustomized && item.customTemplate && (
          <View style={styles.fontInfoRow}>
            <Text variant="caption" color="hint">
              {t('emailTemplate.font')}: {(item.customTemplate.font_family || 'Arial').split(',')[0]}
            </Text>
            <Text variant="caption" color="hint" style={styles.fontInfoSize}>
              {t('emailTemplate.size')}: {item.customTemplate.font_size || '14px'}
            </Text>
          </View>
        )}
      ...
      return (
        <View style={styles.centered}>
          ...
          <Text variant="bodySmall" color="secondary" style={styles.emptyText}>
            {t('emailTemplate.loadingTemplates')}
          </Text>
        </View>
      );
    }
    ...
    <SafeAreaView
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerTitleContainer}>
          <Text variant="h2">{t('emailTemplate.title')}</Text>
