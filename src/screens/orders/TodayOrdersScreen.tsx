...
export const TodayOrdersScreen: React.FC = () => {
  ...
  const handleChat = useCallback(async (order: Order) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : t('errors.failedToOpenChat');
      showAlert({
        type: 'error',
        ...
        <View style={[styles.summaryCard, { backgroundColor: isDark ? themeColors.cardElevated : themeColors.card }]}>
          <View style={styles.summaryStatsRow}>
            <View style={styles.summaryStatItem}>
              ...
              <Text style={[styles.summaryStatLabel, { color: colors.secondary.main }]}>{t('orders.today.ordered')}</Text>
            </View>
            ...
            <View style={styles.summaryStatItem}>
              ...
              <Text style={[styles.summaryStatLabel, { color: colors.success.main }]}>{t('orders.today.delivered')}</Text>
            </View>
            <View style={[styles.summaryStatDivider, { backgroundColor: themeColors.border }]} />
            <View style={styles.summaryStatItem}>
              ...
              <Text style={[styles.summaryStatLabel, { color: colors.warning.main }]}>{t('orders.today.left')}</Text>
            </View>
            <View style={[styles.summaryStatDivider, { backgroundColor: themeColors.border }]} />
            <View style={styles.summaryStatItem}>
              ...
              <Text style={[styles.summaryStatLabel, { color: colors.primary.main }]}>{t('orders.today.progress')}</Text>
            </View>
          </View>
  ...
  const renderEmpty = () => {
    ...
    return (
      <View style={styles.emptyWrap}>
        ...
        <Text style={[styles.emptyTitle, { color: themeColors.text.primary }]}>{t('orders.today.noActiveOrders')}</Text>
        <Text style={[styles.emptySub, { color: themeColors.text.secondary }]}>{t('orders.today.noInProgressOrders')}</Text>
      </View>
    );
  };
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: themeColors.background }]} edges={['top', 'bottom']}>
        ...
        <View style={styles.header}>
          ...
          <Text variant="h2">{t('orders.today.title')}</Text>
          <TouchableOpacity
        ...
        <View style={styles.errorWrap}>
          ...
          <Text style={[styles.errorTxt, { color: themeColors.text.primary }]}>{error || t('errors.failedToLoadOrders')}</Text>
          <Text style={[styles.retryTxt, { color: colors.primary.main }]} onPress={handleRefresh}>{t('orders.today.tapToRetry')}</Text>
        </View>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: themeColors.background }]} edges={['top', 'bottom']}>
      ...
      {isLoading ? (
        <View style={[styles.loadingWrap, { backgroundColor: themeColors.background }]}>
          <TruckLoader size={120} message={t('orders.list.loadingOrders')} color={isDark ? 'light' : 'dark'} />
        </View>
      ) : (
        <>
