...
const LOAD_STATUS_CONFIG: Record<string, { color: string; icon: string; labelKey: string }> = {
  pending: { color: colors.grey[50], icon: 'clock-outline', labelKey: 'orderCard.status.pending' },
  ticketed: { color: colors.trackingStatus.ticketed, icon: 'ticket-outline', labelKey: 'orderCard.status.ticketed' },
  loading: { color: colors.trackingStatus.loading, icon: 'package-variant', labelKey: 'orderCard.status.loading' },
  loaded: { color: colors.trackingStatus.loaded, icon: 'package-variant-closed', labelKey: 'orderCard.status.loaded' },
  to_job: { color: colors.trackingStatus.toJob, icon: 'truck-fast', labelKey: 'orderCard.status.toJob' },
  at_job: { color: colors.trackingStatus.atJob, icon: 'map-marker-check', labelKey: 'orderCard.status.atJob' },
  pouring: { color: colors.trackingStatus.pouring, icon: 'water', labelKey: 'orderCard.status.pouring' },
  begin_pour: { color: colors.trackingStatus.pouring, icon: 'water', labelKey: 'tickets.detail.beginPour' },
  poured: { color: colors.trackingStatus.poured, icon: 'water-check', labelKey: 'orderCard.status.poured' },
  washing: { color: colors.trackingStatus.washing, icon: 'water-pump', labelKey: 'orderCard.status.washing' },
  to_plant: { color: colors.trackingStatus.toPlant, icon: 'arrow-u-left-top', labelKey: 'orderCard.status.toPlant' },
  at_plant: { color: colors.trackingStatus.atPlant, icon: 'home-circle', labelKey: 'orderCard.status.atPlant' },
  cancelled: { color: colors.error.main, icon: 'close-circle', labelKey: 'orders.cancelled' },
  voided: { color: colors.error.main, icon: 'close-circle', labelKey: 'orderCard.status.voided' },
};
...
export const ScheduledLoadsBottomSheet: React.FC<ScheduledLoadsBottomSheetProps> = ({
  ...
  const subtitle = t('orders.detail.loadsCompleted', { completed: completedLoads, total: displayTotal });
    ...
    return (
      ...
      >
            ...
            {load.load_status && (
              <View style={[styles.loadStatusTag, {
                ...
                <Text
                  ...
                  numberOfLines={1}
                >
                  {LOAD_STATUS_CONFIG[load.load_status_code || '']?.labelKey
                    ? t(LOAD_STATUS_CONFIG[load.load_status_code || ''].labelKey)
                    : load.load_status}
                </Text>
              </View>
            )}
        ...
        <View style={[styles.loadTimeRow, {
          ...
          <View style={styles.loadTimeBlock}>
            <Text style={[styles.loadTimeBlockLabel, { color: isDark ? colors.grey[40] : themeColors.text.hint }]}>{t('orderCard.status.atJob')}</Text>
            <Text style={[styles.loadTimeBlockValue, { color: themeColors.text.primary }]}>
          ...
          <View style={styles.loadTimeBlock}>
            <Text style={[styles.loadTimeBlockLabel, { color: isDark ? colors.grey[40] : themeColors.text.hint }]}>{t('orders.product.unload')}</Text>
            <Text style={[styles.loadTimeBlockValue, { color: themeColors.text.primary }]}>
          ...
          <View style={[styles.loadInfoRow, { borderTopColor: isDark ? themeColors.border : colors.grey[10] }]}>
            {load.actual_wash_time && (
              <View style={styles.loadInfoItem}>
                <Text style={[styles.loadInfoLabel, { color: isDark ? colors.grey[40] : colors.grey[50] }]}>{t('orders.product.wash')}</Text>
            ...
            {load.actual_at_plant_time && (
              <View style={styles.loadInfoItem}>
                <Text style={[styles.loadInfoLabel, { color: isDark ? colors.grey[40] : colors.grey[50] }]}>{t('tickets.detail.return')}</Text>
                <View style={[styles.loadInfoTag, {
                  backgroundColor: isDark ? colors.grey[80] : colors.grey[8],
                  borderWidth: 1,
                  borderColor: isDark ? colors.grey[50] : colors.grey[15]
  ...
  const renderHeader = useCallback(() => (
    <View style={styles.summaryRow}>
      <View style={[styles.summaryCard, { backgroundColor: isDark ? themeColors.cardElevated : colors.grey[5] }]}>
        ...
        <Text style={[styles.summaryLabel, { color: isDark ? colors.grey[25] : themeColors.text.secondary }]}>{t('summary.total')}</Text>
      </View>
      <View style={[styles.summaryCard, { backgroundColor: isDark ? themeColors.cardElevated : colors.grey[5] }]}>
      ...
      <View style={[styles.summaryCard, { backgroundColor: isDark ? themeColors.cardElevated : colors.grey[5] }]}>
        ...
        <Text style={[styles.summaryLabel, { color: isDark ? colors.grey[25] : themeColors.text.secondary }]}>{t('orderCard.status.pending')}</Text>
      </View>
    </View>
  ...
  return (
    <BottomSheet
      visible={visible}
      onClose={onClose}
      title={t('orders.detail.scheduledLoads')}
      subtitle={subtitle}
