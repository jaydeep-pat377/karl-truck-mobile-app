...
export const BiometricToggleItem: React.FC<BiometricToggleItemProps> = ({
  ...
  const handleToggle = async () => {
    ...
    try {
      ...
      onError?.(error?.message || t('common.errorOccurred'));
    } finally {
      setIsToggling(false);
    }
  };
    ...
    <TouchableOpacity
      ...
      <View style={styles.content}>
        <Text variant="bodySmall" style={{ fontWeight: '600' }}>
          {isAvailable ? getBiometryTypeName() : t('settingsExtra.biometricAuthentication')}
        </Text>
        <Text variant="caption" color={isAvailable ? 'secondary' : 'error'}>
          {isAvailable
            ? t('settingsExtra.unlockAppWith', { method: getBiometryTypeName().toLowerCase() })
            : t('settingsExtra.biometricNotSetUp')}
        </Text>
      </View>
