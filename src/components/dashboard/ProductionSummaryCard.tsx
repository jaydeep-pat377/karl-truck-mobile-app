...
export const ProductionSummaryCard: React.FC<ProductionSummaryProps> = ({
  ...
  const resolvedTitle = title || t('summary.readyMixProducer');
  ...
  const tabs: { key: SummaryTabType; label: string; count: number }[] = [
    { key: 'company', label: t('summary.company'), count: companies.length },
    ...(showRegion ? [{ key: 'region' as SummaryTabType, label: t('summary.region'), count: regions.length }] : []),
    { key: 'plant', label: t('summary.plant'), count: plants.length },
  ];

  const typeLabelByKey = (type: 'company' | 'region' | 'plant'): string => {
    if (type === 'company') return t('summary.company');
    if (type === 'region') return t('summary.region');
    return t('summary.plant');
  };
      ...
      >
        ...
        <View style={[styles.statsRow, { backgroundColor: isDark ? colors.dark.surface : colors.grey[3] }]}>
          <View style={styles.statItem}>
            ...
            <View style={styles.statContent}>
              <Text style={[styles.statLabel, { color: themeColors.text.secondary }]} numberOfLines={1}>{t('summary.total')}</Text>
          ...
          <View style={styles.statItem}>
            ...
            <View style={styles.statContent}>
              <Text style={[styles.statLabel, { color: themeColors.text.secondary }]} numberOfLines={1}>{t('summary.active')}</Text>
          ...
          <View style={styles.statItem}>
            ...
            <View style={styles.statContent}>
              <Text style={[styles.statLabel, { color: themeColors.text.secondary }]} numberOfLines={1}>{t('summary.cancelled')}</Text>
        ...
        <View style={styles.productionSection}>
          <View style={styles.productionHeader}>
            <Text style={[styles.productionTitle, { color: themeColors.text.primary }]}>{t('summary.productionAndDelivery')}</Text>
            <Text style={[styles.productionQty, { color: themeColors.text.secondary }]} numberOfLines={1}>
              {formatQty(item.deliveredQty)} {t('summary.of')} {formatQty(item.totalQty)} {t('units.cy')}
            </Text>
          </View>
          ...
          <View style={styles.progressFooter}>
            <View style={styles.deliveredRow}>
              ...
              <Text style={[styles.deliveredText, { color: themeColors.text.secondary }]}>
                {t('summary.delivered')}: {formatQty(item.deliveredQty)} {t('units.cy')}
              </Text>
            </View>
