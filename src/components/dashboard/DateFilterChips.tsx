...
const I18N_TO_DATE_LOCALE: Record<string, string> = {
  en: 'en-US',
  es: 'es-ES',
  'fr-CA': 'fr-CA',
};
...
const formatDateFromString = (dateString: string, locale: string): string => {
  const date = new Date(dateString + 'T00:00:00');
  return date.toLocaleDateString(locale, { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' });
};

const formatDateShortFromString = (dateString: string, locale: string): string => {
  const date = new Date(dateString + 'T00:00:00');
  return date.toLocaleDateString(locale, { month: 'short', day: 'numeric', year: 'numeric' });
};

const formatSelectedDateShort = (date: Date, locale: string): string => {
...
export const DateFilterChips: React.FC<DateFilterChipsProps> = ({
  ...
  const filterOptions: { key: DateFilter; label: string }[] = [
    ...
    { key: 'tomorrow', label: t('dateFilter.tomorrow') },
    { key: 'next_week', label: t('dateFilter.nextWeek') },
  ...
  const formatDisplayDate = (date: Date): string => {
    ...
    if (isToday) return `${t('dateFilter.todayPrefix')}, ${weekday} - ${monthDay}, ${year}`;
    if (isTomorrow) return `${t('dateFilter.tomorrowPrefix')}, ${weekday} - ${monthDay}, ${year}`;
          ...
          <View style={styles.calendarChipContent}>
            ...
            {isCalendarSelected && selectedDate && (
              <Text style={styles.calendarDateText}>
                {formatSelectedDateShort(selectedDate, dateLocale)}
              </Text>
            )}
          </View>
        </TouchableOpacity>
          ...
          <View style={styles.dateRowLeft}>
            ...
            <Text style={[styles.dateText, { color: themeColors.text.primary }]}>
              {dateRange.start_date === dateRange.end_date
                ? formatDateFromString(dateRange.start_date, dateLocale)
                : `${formatDateShortFromString(dateRange.start_date, dateLocale)} - ${formatDateShortFromString(dateRange.end_date, dateLocale)}`}
            </Text>
          </View>
        </View>
      )}
